...
of your choice.
    ...
    halfling = Race(name='Halfling', details="""
Halfling
~~~~~~~~
...
expressions, Dwarvish military phrases, and so on.
    ...
    dragonborn = Race(name='Dragonborn', details="""
Dragonborn
~~~~~~~~~~
...
-  **Music Box.** When opened, this music box plays a single song at a
   ...
   when it is closed.
    ...
    half_elf = Race(name='Half-Elf', details="""
Half-Elf
~~~~~~~~
...
extra language of your choice.
    ...
    half_orc = Race(name='Half-Orc', details="""
Half-Orc
~~~~~~~~
...
own but is written in the Dwarvish script.
    ...
    tiefling = Race(name='Tiefling', details="""
Tiefling
~~~~~~~~
...
**Languages.** You can speak, read, and write Common and Infernal.
    ...
    session.merge(dwarf)
    session.merge(elf)
    session.merge(halfling)
    session.merge(human)
    session.merge(dragonborn)
    session.merge(gnome)
    session.merge(half_elf)
    session.merge(half_orc)
    session.commit()
